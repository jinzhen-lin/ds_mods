...
function TMI_Menubar:InitMenu()
    local fontsize = 36
    local spacing = 40
    local left = self.sidebar_width - self.owner.shieldsize_x * .5 + 15
    local right = self.owner.shieldsize_x * .5 - 15
...
function TMI_Menubar:InitSearch()
    ...
    self.searchbarbutton:SetFont(DEFAULTFONT)
    ...
    self.searchhelptip:SetFont(DEFAULTFONT)
    ...
    self.searchhelptip:SetOnClick( function()
        self:SearchKeyWords()
    end)
    ...
    self.searchtext = self.searchshield:AddChild(Text(DEFAULTFONT, self.search_fontsize))
...
function TMI_Menubar:SearchKeyWords()
    ...
    self.searchbar.searchhelptip = self.searchhelptip
...
function TMI_Menubar:OnRawKey(key, down)
    if TMI_Menubar._base.OnRawKey(self, key, down) then
        return true
    end
end
