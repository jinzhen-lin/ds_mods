...
local function BeavernessSet()
	if TheInput:IsKeyDown(KEY_CTRL) then
		SendCommand(string.format('local player = %s local v = player.components.beaverness if player and not player:HasTag("playerghost") and v then v:SetPercent(0) end', GetCharacter()))
	else
		SendCommand(string.format('local player = %s local v = player.components.beaverness if player and not player:HasTag("playerghost") and v then v:SetPercent(1) end', GetCharacter()))
	end
end
